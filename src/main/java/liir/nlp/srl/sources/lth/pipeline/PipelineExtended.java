 ...
 */
public class PipelineExtended extends SemanticRoleLabeler{
    ...
    private PipelineExtended(FeatureGenerator fg, Map<Step, FeatureSet> featureSets, Iterable<Sentence> sentences){
...
//		extractLabels(sentences, featureSets.get(Step.pd).POSPrefixes); //TODO, sort of nasty.
    ...
    private PipelineExtended(FeatureGenerator fg,PredicateReference predicateReference,List<String> argLabels, Map<Step, FeatureSet> featureSets){
        ...
        for(Step step:Step.values()){
            if(featureSets.containsKey(step)){
                switch(step){
                    ...
                    case pd: steps.add(new PredicateDisambiguatorExtended(featureSets.get(Step.pd),predicateReference)); break;
                    case ai: steps.add(new ArgumentIdentifierExtended(featureSets.get(Step.ai))); break;
                    case ac: steps.add(new ArgumentClassifierExtended(featureSets.get(Step.ac),argLabels)); break;
                }
            }
        }
    }
    ...
    public void predicateIdentify(Sentence s) {
        steps.get(0).parse(s);
    }

    public HashMap<String,Double> PredicateDisambiguatorProb(Predicate pred, Sentence s) {
        HashMap<String,Double> 	senses = new HashMap<String,Double> ();
        senses= ((PredicateDisambiguatorExtended)steps.get(1)).parseProb(pred);
        return senses;
    }

    /***
     * return scores regarding all possible labels for each word
     * the first value is the score for argument identification
     * @param pred
     * @param sense
     * @param s
     * @return
     */
    public ArrayList<ArrayList<Double>> parseProb(Predicate pred, String sense, Sentence s){
        pred.setSense(sense);
        ArrayList<ArrayList<Double>> arrrs = new ArrayList<ArrayList<Double>>();
        for (Word arg : s)
        {
            if (!arg.isBOS())
            {
                ArrayList<Double> arr  = new ArrayList<Double>();
                double d =	((ArgumentIdentifierExtended)steps.get(2)).parseProb(pred, arg);
                HashMap<String,Double> scores=	((ArgumentClassifierExtended)steps.get(3)).parseProb(pred,s, arg);
                arr.add(d);
                for (String lbl : argLabels){
                    Boolean found = false;
                    for (HashMap.Entry<String,Double> p : scores.entrySet()){
                        if (p.getKey().equals(lbl))
                        {
                            arr.add(p.getValue());
                            found=true;
                        }
    ...
    public static PipelineExtended trainNewPipeline(Iterable<Sentence> sentences,FeatureGenerator fg,ZipOutputStream zos,Map<Step,FeatureSet> featureSets) throws IOException {
        //Then create the arglabel list and predicatereference
        PipelineExtended pipeline=new PipelineExtended(fg,featureSets,sentences);
    ...
    public static PipelineExtended trainNewPipeline(Iterable<Sentence> sentences,Map<Step,File> featureFiles,ZipOutputStream zos,BrownCluster bc) throws IOException {
        ...
        PipelineExtended pipeline=trainNewPipeline(sentences,fg,zos,featureSets);
        pipeline.fg=fg;
        return pipeline;
    }

    @SuppressWarnings("unchecked")
    public static PipelineExtended fromZipFile(ZipFile zipFile,Step[] steps) throws IOException, ClassNotFoundException{
        ...
        PipelineExtended pipeline=new PipelineExtended(fg,predicateReference,argLabels,featureSets);
        //Load models
